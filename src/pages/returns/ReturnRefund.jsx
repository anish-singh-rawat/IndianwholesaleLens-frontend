  ...
  if (successData) {
    return (
      <div className="w-full min-h-[70vh] flex flex-col items-center justify-center animate-in fade-in duration-500 py-10">
        <div className="rounded-[2.5rem] p-10 max-w-xl w-full text-center relative overflow-hidden"
        style={{ background: 'color-mix(in oklab, var(--card) 85%, transparent)', backdropFilter: 'blur(24px)', border: '1px solid rgba(255,255,255,0.12)' }}>
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-erp-accent to-erp-primary"></div>

          <h2 className="text-3xl font-bold mb-2" style={{ color: 'var(--foreground)' }}>Return & Refund Successful</h2>
          <p className="text-sm mb-6" style={{ color: 'var(--muted-foreground)' }}>Your transaction has been processed successfully.</p>

          <div className="w-48 h-48 mx-auto mb-8 rounded-full flex items-center justify-center relative" style={{ background: 'rgba(59,130,246,0.12)' }}>
            <div className="absolute inset-0 rounded-full animate-pulse" style={{ border: '1px solid rgba(59,130,246,0.20)' }}></div>
            <Icon icon="mdi:emoticon-happy-outline" className="text-9xl text-erp-accent animate-bounce duration-1000" />
          </div>

          <div className="rounded-2xl p-4 mb-8 text-left space-y-2" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div className="flex justify-between text-xs"><span style={{ color: 'var(--muted-foreground)' }}>Request Name</span><span className="font-bold" style={{ color: 'var(--foreground)' }}>{formData.name}</span></div>
            <div className="flex justify-between text-xs"><span style={{ color: 'var(--muted-foreground)' }}>Refund Amount</span><span className="font-bold text-erp-accent">₹ {formData.refundAmount || "0.00"}</span></div>
            <div className="flex justify-between text-xs"><span style={{ color: 'var(--muted-foreground)' }}>Method</span><span className="font-bold" style={{ color: 'var(--foreground)' }}>{formData.refundMethod}</span></div>
          </div>
  ...
  return (
    <div className="w-full flex flex-col gap-6 animate-in fade-in duration-500 px-4 md:px-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div>
          <h1 className="text-2xl md:text-3xl font-bold" style={{ color: 'var(--foreground)' }}>Return & Refund</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--muted-foreground)' }}>Manage return requests and refunds</p>
        </div>
      ...
      {activeTab === "form" ? (
            ...
            <div className="rounded-[2.5rem] p-8 space-y-8" style={{ background: 'color-mix(in oklab, var(--card) 75%, transparent)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.09)' }}>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <input name="condition" value={formData.condition} onChange={handleChange} className={inputCls} style={inputSty} placeholder="e.g. Scratched, Damaged" />
                  </div>
                </div>
                <div>
                  <label className={labelCls} style={labelSty}>Reason for Return</label>
                  <select name="reasonForReturn" value={formData.reasonForReturn} onChange={handleChange} className={`${inputCls} appearance-none`} style={inputSty}>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <button type="button" onClick={addItemRow} className="text-xs font-bold text-erp-accent flex items-center gap-1 hover:underline">
              ...
              {/* Refund Info */}
              <div className="space-y-4">
                <span className="bg-erp-accent/10 text-erp-accent font-bold px-5 py-2 rounded-full text-xs flex items-center gap-2 w-max">
                  <Icon icon="mdi:keyboard-backspace" className="text-base" /> Refund
                </span>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  <div>
                    <label className={labelCls} style={labelSty}>Refund Amount *</label>
                    <input type="number" name="refundAmount" value={formData.refundAmount} onChange={handleChange} className={inputCls} style={inputSty} placeholder="₹ 0.00" />
                  </div>
                  <div>
                    <label className={labelCls} style={labelSty}>Method</label>
                    <select name="refundMethod" value={formData.refundMethod} onChange={handleChange} className={`${inputCls} appearance-none`} style={inputSty}>
                  ...
                  <div>
                    <label className={labelCls} style={labelSty}>Loyalty Points (if applicable)</label>
                    <input type="number" name="loyaltyPoints" value={formData.loyaltyPoints} onChange={handleChange} className={inputCls} style={inputSty} placeholder="Points" />
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    <label className={labelCls} style={labelSty}>Remark / Reason</label>
                    <input name="remark" value={formData.remark} onChange={handleChange} className={inputCls} style={inputSty} placeholder="Enter remark" />
                  </div>
                  <div>
                    <label className={labelCls} style={labelSty}>Gift Voucher Doc (Optional)</label>
                    <div onClick={() => voucherRef.current?.click()} className="flex items-center gap-3 border-dashed rounded-full px-5 py-2.5 text-sm cursor-pointer hover:bg-erp-accent/5 transition-all"
                      style={{ border: '1px dashed rgba(255,255,255,0.15)', color: 'var(--muted-foreground)' }}>
                      <Icon icon="mdi:file-document-outline" className="text-lg" style={{ color: 'var(--muted-foreground)' }} />
          ...
          <div className="space-y-8">
            <div className="rounded-[2.5rem] p-8 sticky top-8 space-y-6" style={{ background: 'color-mix(in oklab, var(--card) 75%, transparent)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.09)' }}>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold bg-erp-accent text-white px-4 py-1 rounded-full shadow-lg shadow-erp-accent/20">{photos.length}/10</span>
              </div>
        ...
        <div className="rounded-[2.5rem] p-8 space-y-6 pb-16" style={{ background: 'color-mix(in oklab, var(--card) 75%, transparent)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.09)' }}>
          <form onSubmit={handleSearch} className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
            ...
            <div>
              <button type="submit" className="w-full bg-erp-accent hover:bg-erp-accent/90 text-white font-bold py-3 rounded-full text-xs transition-all flex items-center justify-center gap-2">
                ...
                {listData.length > 0 ? (
                  listData.map((row) => (
                    <tr key={row._id} className="hover:bg-erp-accent/[0.04] transition-colors" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      ...
                      <td className="p-4" style={{ color: 'var(--muted-foreground)' }}>{row.dateOfPurchase ? row.dateOfPurchase.split("T")[0] : "-"}</td>
                      <td className="p-4" style={{ color: 'var(--muted-foreground)' }}>{row.itemType || "-"}</td>
                      <td className="p-4 font-bold text-erp-accent">₹ {row.refundAmount}</td>
                      <td className="p-4">
                  ...
                  <tr>
                    <td colSpan={7} className="p-8 text-center opacity-40">No return requests found</td>
                  </tr>
                )}
              </tbody>
            </table>
